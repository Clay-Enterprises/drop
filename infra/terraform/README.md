...
The existing `drop-content` custom domain is read and asserted instead of managed. Cloudflare's provider cannot import an existing R2 custom-domain resource. Small idempotent helpers keep `drop-control` free of custom domains and register the account-level `workers.dev` name because those absent/account-level states are not represented by the provider.
